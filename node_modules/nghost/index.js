...
var nghost = function (option = options) {
...
function getGenericRequest(req, res) {
    ...
    if (pathName.length == 0) {
        ...
        checkReqAndSend(pathName.join('/'), res);
}
...
function responseText(res, file, contentType) { response(res, file, contentType, options.encode); }

function responseBinary(res, file, contentType) { response(res, file, contentType); }

var response = function (res, file, contentType, encode) {
    if (file != "404") {
        res.writeHead(200, { "Content-type": contentType });
        if (encode) {
            var stream = fs.createReadStream(path.join(options.publicFolder, file), encode)
                .pipe(stringReplaceStream("localhost:" + options.port, options.hostname + ":" + options.port));
        } else {
            var stream = fs.createReadStream(path.join(options.publicFolder, file))
                .pipe(stringReplaceStream("localhost:" + options.port, options.hostname + ":" + options.port));
        }
        stream.pipe(res);
    } else {
...
var addIfNotExist = function (obj1, obj2) {
    //Fetch Contents
    if (obj2.contents) {
        var result;
        for (ctn in obj1.contents) {
            result = obj2.contents.filter(obj => {
    ...
    if (obj2.sessionKey) {
        ...
        for (ctn in obj1.sessionKey) {
            result = obj2.sessionKey.filter(obj => {
